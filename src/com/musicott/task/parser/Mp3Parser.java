...
public class Mp3Parser {
	...
	public static Track parseMp3File(final File fileToParse) {
		...
		try {
			...
			mp3Track.setFileFolder(new File(fileToParse.getParent()).getAbsolutePath());
			mp3Track.setFileName(fileToParse.getName());
			checkCover(mp3Track);
			mp3Track.getIsInDisk().set(true);
			mp3Track.getSize().set((int) (fileToParse.length()));
			...
			if(mp3File.hasId3v2Tag())
				...
				if(mp3File.hasId3v1Tag())
					...
					mp3Track.getName().set(fileToParse.getName());
				}
		} catch (Exception e) {
...
//		track.getYear().set(Integer.valueOf(tag.getYear()));
		...
		//TODO Missing set DiscNumber
		//TODO Parse correctly tag.getDate() to track.getDateModified()
	}
